...
★ 前置三 gate（產頁前，任一不綠 exit 1 且零產出）：
  ...
  ③ golden 全綠（tests/golden_driver_stats.json 逐欄凍結值）
...
# 哪些季有分站頁（明細列可深連分站頁）——須與 seasons 管線 --rounds-for 一致；其餘季明細連季總覽。
ROUND_YEARS = {2002, 2026}
...
def _computed_row(did, con):
    car = fs.driver_career_db(did, con)
...
def gate_golden(golden_path=None, con=None):
    """f1stats DB 現值逐欄比對 golden；任一 diff → False（需人工重新核准才更新 golden）。"""
    ...
    try:
        ...
        for did in CHAMPION_IDS:
            want, got = golden[did], _computed_row(did, con)
