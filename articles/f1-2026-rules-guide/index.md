...
subtitle: "2026 年是 F1 繼 2014 年之後又一次大規模改規：DRS 走入歷史、由 Overtake Mode 接棒，前後翼可動的主動空力上路，動力單元電動輸出提高到接近一半，車變小變輕，還多了凱迪拉克這支第 11 隊。本文逐項拆解每一條新規則的機制與半季下來的實際效果，規格數字對照 FIA 規章與賽事官方說明查證，兩者寫法不同處分別標明出處。"
lede: "如果你今年才回頭看 F1，會發現轉播裡少了熟悉的 DRS 字樣，多了 Overtake、Boost 這些新詞；前後翼會在直線上放平，圈速慢了一些，開季前幾站的超車卻多了一倍。這些變化全部來自 2026 年生效的新技術規則。這篇指南把改了什麼、為什麼改、半季跑下來效果如何，一次講清楚。"
---
...
- **Straight Mode（直線模式）**：翼片打開、阻力大減，在 FIA 賽前公布的啟用區段內，系統通知已開放時由車手啟用，不限車、不限圈（formula1.com 2024 年的說法是預計開放給 3 秒以上的直線；低抓地條件下規章只允許前翼打開的「部分啟用」），入彎前切回 Corner Mode。

規則制定期這兩個模式叫 X-mode 與 Z-mode，你在 2024、2025 年的報導裡看到的是舊名；季前 FIA 與 F1 官方把對外用語統一改成 Straight／Corner Mode。

它與 DRS 最大的差別有兩點。其一，DRS 只動後翼，主動空力是**前後翼連動**切換。其二，DRS 是「距前車 1 秒內才能開」的追擊工具，Straight Mode 則是人人可用的減阻工具。它本身不是超車輔助，超車輔助另有其人，就是下一節的主角。

## Overtake Mode：DRS 的接班人

DRS 用了 15 年（2011-2025），2026 年正式退役。接棒的超車輔助叫 **Overtake Mode**：正賽中在賽道指定偵測點距前車 1 秒以內（1 秒是 formula1.com 的說法；規章寫的是 FIA 賽前公布的「偵測間距」），下一圈就能啟用，效果是每圈最多額外回充 0.5 MJ（規章 C5.2.10 條），並讓電動馬達的輸出曲線延伸到更高車速。簡單說，別人的電已經開始衰減時，你的還在全力放電。
...
2026 年的車全面「瘦身」：軸距上限縮短 200 mm 到 3400 mm（規章 Section C 第 C2.3.3 條）。寬度方面，formula1.com 入門指南寫的是底板寬度縮短 100 mm 到 1.9 m；規章第 C2.3.1 條則規定，輪胎、輪圈及輪圈附件以外，車上任何部分距中心線不得超過 950 mm，也就是不含輪胎的車身寬度上限 1900 mm。最低重量方面，formula1.com 寫的是 768 kg、比前一年的 800 kg 輕；規章第 C4.1 條的寫法是 724 kg 加上標稱輪胎重量（衝刺排位與排位賽為 726 kg 加標稱輪胎重量）。輪胎維持 18 吋，但前胎變窄 25 mm、後胎變窄 30 mm。formula1.com 2024 年的空力規則解說寫到，新規則讓下壓力減少 30%、阻力減少 55%。
...
衝刺賽 2026 年共 6 場：中國、邁阿密、加拿大（首辦）、英國（回歸）、荷蘭（首辦）、新加坡（首辦），前 4 場已跑完。積分制度沒有改變：領先者完成預定距離 75% 以上時，正賽前 10 名依 25-18-15-12-10-8-6-4-2-1 計分（距離不足時改用較低的積分表）；衝刺賽完成 50% 以上時，前 8 名依 8 到 1 計分（Section A 第 A2.2 條；兩種賽事都要求領先者至少連續完整跑完兩圈、且期間沒有安全車或虛擬安全車程序，否則不發分）；最快單圈加分早在 2025 年就取消了。比較細的變化是排位賽：因為場上有 22 台車，Q1、Q2 各淘汰 6 台。
...
**第三，能量管理成了新的抱怨來源，FIA 也已出手修正**。車手需要在高速彎前收油回充（lift-and-coast），維斯塔潘（Max Verstappen）等多位車手公開表達不滿，車速差擴大也帶來安全疑慮。FIA 在 4 月底與各方拍板季中修正，自邁阿密站起實施。FIA 公告在排位賽項下列出：每圈最大回充由 8 MJ 降為 7 MJ、superclip 峰值功率由 250 kW 上調至 350 kW（公告註明正賽同樣適用）、可適用更低能量上限的場次由 8 站增為 12 站；正賽則把 Boost 的最大功率增幅限制在 +150 kW。現行規章（Section C Issue 20 第 C5.2.10 條）的寫法是：每圈回充上限基準為 8.5 MJ，FIA 可在特定賽事降到 7 MJ，排位賽與衝刺排位賽可再下修。修正緩解了最極端的收油情況，但車手的抱怨沒有完全消失，後續再調的空間仍然存在。
...
沒有。DRS（可變尾翼減阻系統）在 2025 年賽季結束後正式退役，2026 年起由 Overtake Mode 接替超車輔助的角色——原理從「減少後翼阻力」改成「額外回充與更長的電力輸出」。直線減阻的功能則由人人可用的 Straight Mode（主動空力直線模式）承擔。

### Overtake Mode 和 Boost 有什麼不同？

Overtake Mode 在正賽中需要在偵測點距前車 1 秒內（formula1.com 的說法）才能取得，下一圈生效，給的是每圈最多額外 0.5 MJ 的回充與延長的輸出曲線，是「追擊獎勵」；Boost 則是車手隨時可按的按鈕，在任意位置切回最大功率或車隊預設的輸出設定，不需要距離條件（自邁阿密站起，正賽中 Boost 的最大功率增幅限制在 +150 kW）。前者像加長彈匣，後者像手動點放。
